...
export default async function FilmPage({ params }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-green-500 md:w-[800px] sm:w-[400px] h-auto p-8 flex flex-col justify-center items-center absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-[12px]">
        <Link
          href="/films"
          className="absolute top-6 md:top-6 sm:top-4 left-10 md:left-10 sm:left-6 hover:bg-green-600 transition-shadow rounded-full px-3 py-2 flex items-center justify-center "
        >
          <span className="text-white md:text-3xl sm:text-2xl text-center">&larr;</span>{" "}
          
        </Link>
        <h1 className="text-white md:text-3xl sm:text-2xl font-bold mb-6 animate__animated animate__flipInX">{film.title}</h1>

        <div className="grid md:grid-cols-2 sm:grid-cols-1 gap-x-6 md:gap-x-6 sm:gap-x-4 gap-y-4 w-full text-white text-lg animate__animated animate__flipInX">
